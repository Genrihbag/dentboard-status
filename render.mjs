...
<main>
  ...
  <!-- ⚠️ ПОДПИСЬ ПРО ВРЕМЯ ОБЯЗАТЕЛЬНА, А НЕ УКРАШЕНИЕ. Замер идёт с машин GitHub из-за рубежа: в
       него входит дорога (100–150 мс на обмен), TLS в несколько обменов и холодный DNS. Замерено
       28.08.2026: сам сервер отдаёт первый байт за 40–160 мс, а полное время из-за границы —
       780–2842 мс. Без подписи эти числа читаются как «сайт тормозит», и разбор пойдёт искать
       поломку там, где всё исправно. -->
  <p class="dim">
    Время — это полный запрос из-за рубежа: дорога, шифрование и генерация страницы вместе.
    Из России отклик в разы меньше.
  </p>
