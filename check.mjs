...
/**
 * 🔴 ПРОБНОЕ СООБЩЕНИЕ — ПО ЯВНОЙ ПРОСЬБЕ, И ТЕМ ЖЕ КОДОМ, ЧТО ШЛЁТ НАСТОЯЩИЕ.
 *
 ...
 *
 * ⚠️ И ПРОГОН ПРИ ЭТОМ ОБЫЧНЫЙ: цели опрашиваются, история пишется, аватар сводится. Проба —
 * ДОБАВКА к настоящему прогону, а не подмена его: режим, расходящийся с боевым, доказывает только
 * то, что работает сам режим.
 */
if (process.env.TEST_MESSAGE === "true") {
  ...
  const sent = await telegram(
    `🔔 <b>Проба связи</b>
${stamp} МСК · проверено снаружи, с машин GitHub.
` +
      (down.length === 0
...
<a href="${STATUS_PAGE}">Страница состояния</a>`,
  ...
  // Отказ пробы — ОТКАЗ ЗАДАНИЯ. Напечатать «не отправилось» и выйти нулём значило бы завести
  // проверку, которую можно не заметить, — а её ради заметности и заводили.
  if (!sent) {
    console.error("проба связи НЕ доставлена — см. причину выше");
    process.exitCode = 1;
  }
}
